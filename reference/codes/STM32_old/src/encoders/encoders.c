...
static void config_tim_quadrature(TIM_TypeDef *tim) {
    tim->CR1   = 0;
    tim->CCMR1  = TIM_CCMR1_CC1S_0 | TIM_CCMR1_CC2S_0;  /* CC1S=01, CC2S=01 (TI mapping) */
    tim->CCMR1 |= (3u << 4) | (3u << 12);               /* ICxF=3 → fSAMPLE = fDTS, N=4 */
    tim->CCER  = 0;                                     /* both channels rising-edge active */
    tim->SMCR  = TIM_SMCR_SMS_0 | TIM_SMCR_SMS_1;       /* SMS=011 → encoder mode 3 (×4) */
    tim->ARR   = 0xFFFFu;                               /* force TIM2 to wrap like a 16-bit timer
                                                         * so the wrap-correction below handles
                                                         * both TIM2 (32-bit) and TIM3 (16-bit). */
    tim->CNT   = 0;
    tim->CR1  |= TIM_CR1_CEN;
}
