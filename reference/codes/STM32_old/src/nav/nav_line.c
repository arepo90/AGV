...
void nav_line_get(float dt_s, float *v_target, float *omega_target) {
    ...
    /* Setpoint = 0 (line should be centred). Positive position means the line
     * is to the right → robot needs ω < 0 to turn right (CW). With error =
     * setpoint - measurement = -position and Kp > 0, that comes out right. */
