...
#define CMD_LOAD_RAMP_CURVE         0x0Eu   /* upload custom ramp shape — see cmd.c */
...
/* Super-twisting sliding-mode gains (k1, k2) per cascade loop. Slots
 * 0x12/0x15/0x18/0x1B (formerly Kd) are reserved/unused. */
#define PARAM_INNER_K1_LEFT             0x10u
#define PARAM_INNER_K2_LEFT             0x11u
#define PARAM_INNER_K1_RIGHT            0x13u
#define PARAM_INNER_K2_RIGHT            0x14u
#define PARAM_OUTER_LIN_K1              0x16u
#define PARAM_OUTER_LIN_K2              0x17u
#define PARAM_OUTER_ANG_K1              0x19u
#define PARAM_OUTER_ANG_K2              0x1Au
...
#define PARAM_TRAJ_CURV_SLOWDOWN        0x27u   /* pure-pursuit speed-vs-curvature gain (m) */
...
/* Motion profile (ramp) — value is f32 on the wire. Shape param is cast from
 * float → uint via truncation. See ramp.h for shape enum. */
