 ...
 * Each slot reserves PROTO_MAX_FRAME (263 B), so this dominates RAM. 8 slots
 * (≈2.1 KB) absorbs the worst observed burst (telemetry + ACK + a few log
 * entries) and drains in ~11 ms at 921600 baud. Overflows trip
 * LOG_CODE_TX_QUEUE_FULL, visible in telemetry's log_dropped count. */
#define TX_SLOTS  8u
...
/* Single-slot mailbox. If the main loop is slow and another frame lands before
 * the previous one was consumed, the new one overwrites it and s_rx_pkt_dropped
 * bumps. Telemetry-rate traffic shouldn't realistically generate contention. */
...
static bool tx_enqueue(const uint8_t *frame, uint8_t total_len) {
    ...
    if (next == s_tx_tail) {
        ...
        /* No log_record here — log forwarding routes back through tx_enqueue,
         * so a queue-full event would recursively try to enqueue itself. The
         * drop is surfaced via log_dropped_count() in the next telemetry frame. */
        return false;
    }
