 ...
 *    7.  hx711_init                        load cell GPIO (skipped if DISABLE_LOAD_CELLS)
 *    8.  imu_init                          I2C1 + BNO055 fusion (may fail; we continue)
 ...
 *   14.  nav_line_load_calibration_*       load QTR baselines from flash if present
 *   15.  odometry_init                     pose at origin
 *   16.  control_init                      PIDs constructed at config-defined gains
 *   17.  current_monitor_init              overcurrent streak counters reset
 *   18.  cargo_monitor_init                no per-tick state today (stub for future)
 *   19.  proximity_init                    EXTI live; ISR may immediately assert E-STOP
 *   20.  boot_record                       log reset cause now that proximity init is done
 *   21.  system_iwdg_init                  watchdog last; all init must finish within timeout
 * =============================================================================
 */
...
/* Telemetry frame is 128 bytes; layout is documented in architecture.md. */
static void send_telemetry(uint32_t now_ms) {
    ...
    /* Per-wheel target velocities feeding the inner STAs — post-ramp,
     * post-outer-cascade, post-kinematic-split. The values that actually
     * arrive at sta_step() for the inner loop. */
    PUT_F32(control_v_left_target());                /* offset 120 */
    PUT_F32(control_v_right_target());               /* offset 124 */
...
int main(void) {
    system_init();
    log_init();
    motors_init();           /* asserts SLEEP LOW so motors stay safe until first command */
    encoders_init();
    comms_init();
    adc_init();
    ...
    heartbeat_init();
    nav_init();
    /* Must run after nav_init: a missing or corrupt flash record falls back
     * to the compile-time defaults that nav_init just installed. */
    nav_line_load_calibration_from_flash();
    odometry_init();
    ramp_init();             /* must precede control_init: control resets call ramp_reset */
    control_init();
