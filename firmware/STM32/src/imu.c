...
static bool bno055_setup(void) {
    ...
    /* UNIT_SEL: bit 7=0 windows orientation, bit 4=0 temp °C, bit 2=0 deg, bit 1=0 dps, bit 0=0 m/s². */
...
void imu_tick(uint32_t now_ms) {
    ...
    if (i2c_write_then_read(BNO055_I2C_ADDR, BNO055_REG_CALIB_STAT, &cal, 1)) {
        if (cal != s_calib && (cal & 0xC0) != (s_calib & 0xC0)) {
            /* System calibration field changed — log so workstation can prompt
             * the operator to do figure-eight motions if it dropped. */
            log_record(LOG_MOD_IMU, LOG_SEV_INFO,
                       (cal < s_calib) ? LOG_CODE_IMU_CALIB_LOST : LOG_CODE_BOOT,
                       cal);
        }
        s_calib = cal;
    }
...
float imu_yaw_rad(void) {
    ...
    /* BNO055 returns 0..360 — wrap to [-π, +π]. */
    if (y_deg > 180.0f) y_deg -= 360.0f;
    return y_deg * (float)M_PI / 180.0f;
}
