...
/* Lock-free-ish ring: head/tail indices + a critical section around the write.
 * Reader (main loop, single context) drains via log_pop(). Writer can be ISR. */
...
static volatile uint32_t s_dropped = 0;  /* count of overflowed-and-discarded */
...
void log_record(log_module_t module, log_severity_t sev, log_code_t code, uint32_t data) {
    /* Disable IRQs briefly to make head increment + slot write atomic vs. ISRs. */
    uint32_t primask = __get_PRIMASK();
    __disable_irq();

    uint32_t next = (s_head + 1u) % FAULT_LOG_DEPTH;
    if (next == s_tail) {
        /* Full — drop the oldest by advancing tail, then write new entry. */
        s_tail = (s_tail + 1u) % FAULT_LOG_DEPTH;
        s_dropped++;
    }

    log_entry_t *e = &s_buf[s_head];
    e->timestamp_ms = system_now_ms();
    e->code         = code;
    e->severity     = sev;
    e->module       = module;
    e->data         = data;
