...
/* SCK high pulse must be < 60 µs (else HX711 enters powerdown). With a 48 MHz
 * core a few NOPs gives clean ~100 ns edges — well clear of both the 0.2 µs
 * minimum and the 60 µs maximum. */
...
void hx711_init(void) {
    ...
    /* DOUT lines are inputs (mode 00, the reset default), no pulls — the HX711
     * actively drives them. We're already at default; nothing to set. */
...
void hx711_tick(uint32_t now_ms) {
    ...
    if (!dout_all_ready()) {
        /* Long-running not-ready means a chip is misbehaving. Log once. */
        if ((now_ms - s_last_read_ms) > HX711_TIMEOUT_US * 1000u
            && (now_ms - s_last_read_ms) < (period_ms_for_state() + 50u)) {
    ...
    if (s_tare_active) {
        ...
        if (s_tare_samples_left == 0) {
            ...
            s_tare_active = false;
            log_record(LOG_MOD_HX711, LOG_SEV_INFO, LOG_CODE_HX711_OUT_OF_RANGE, 0);
        }
    }
#endif
}
