...
function StandbyPanel({ theme, agv, connected, telem }) {
  ...
  const Btn = ({ onClick, children, color = theme.accent, dis }) => (
    <button onClick={onClick} disabled={dis || disabled}
      style={{
        padding: '10px 14px', borderRadius: '8px', fontSize: '11px', fontFamily: theme.monoFont, fontWeight: 600,
        border: `1.5px solid ${color}`, background: color + '15', color,
        cursor: (dis || disabled) ? 'not-allowed' : 'pointer', opacity: (dis || disabled) ? 0.45 : 1,
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <Btn onClick={() => send(() => agv.cmdStartTare())}>⊘ Tare (zero load cells)</Btn>
      ...
      <div>
        ...
        <Btn onClick={() => send(() => agv.cmdResetOdometry())}>⟲ Reset odometry (x=y=θ=0)</Btn>
      ...
      <div style={{ gridColumn: '1 / -1' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          <Btn onClick={() => { send(() => agv.cmdQtrCalibrate(0)); setQtrPhase('sweeping'); }} dis={qtrPhase === 'sweeping'}>▶ Begin sweep</Btn>
          <Btn color={theme.success} onClick={() => { send(() => agv.cmdQtrCalibrate(1)); setQtrPhase('idle'); }} dis={qtrPhase !== 'sweeping'}>✓ Save + persist</Btn>
          <Btn color={theme.warn} onClick={() => { send(() => agv.cmdQtrCalibrate(2)); setQtrPhase('idle'); }} dis={qtrPhase !== 'sweeping'}>✗ Cancel</Btn>
          <Btn color={theme.danger} onClick={() => send(() => agv.cmdQtrCalibrate(3))}>⌫ Reset to defaults</Btn>
        </div>
        <div style={{ fontSize: '10px', color: theme.muted, fontFamily: theme.monoFont, lineHeight: 1.5, marginTop: '8px' }}>
          {qtrPhase === 'sweeping'
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
      ...
      <div style={{ gridColumn: '1 / -1', display: 'flex', gap: '14px', background: theme.bg, border: `1px solid ${theme.border}`, borderRadius: '8px', padding: '10px 14px' }}>
        <span style={{ fontSize: '10px', fontFamily: theme.monoFont, color: theme.muted, textTransform: 'uppercase', letterSpacing: '0.1em' }}>Line error</span>
        <span style={{ fontSize: '12px', fontFamily: theme.monoFont, color: theme.accent, fontWeight: 700 }}>{fmt(telem.lineError, 3)}</span>
        <span style={{ marginLeft: 'auto', fontSize: '10px', fontFamily: theme.monoFont, color: theme.muted }}>PID gains live-tunable on the PID tab</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
      <div style={{ background: theme.warn + '12', border: `1px solid ${theme.warn}55`, borderRadius: '8px', padding: '12px 14px', fontSize: '11px', fontFamily: theme.monoFont, color: theme.fg, lineHeight: 1.5 }}>
        Load and edit waypoints on the <strong style={{ color: theme.warn }}>Trajectory</strong> tab. The controls below tune execution.
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
            <span style={{ fontSize: '10px', fontFamily: theme.monoFont, color: theme.muted, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Lookahead</span>
...
function HeatLegend({ maxVal }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '28px', flexShrink: 0, height: '158px' }}>
      <span style={{ fontSize: '8px', fontFamily: 'monospace', color: 'rgba(255,255,255,0.55)', lineHeight: 1, marginBottom: '3px', whiteSpace: 'nowrap' }}>{maxVal.toFixed(1)} kg</span>
...
function WeightHeatmap({ fl, fr, rl, rr, cogX, cogY }) {
  const maxVal = Math.max(fl, fr, rl, rr, 0.001);
  ...
  // cogX/cogY: -100…+100 where 0 = centre
  const cx = `${(50 + cogX / 2).toFixed(2)}%`;
  const cy = `${(50 + cogY / 2).toFixed(2)}%`;
    ...
    <div style={{ padding: '16px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', overflowY: 'auto', height: '100%', boxSizing: 'border-box', alignContent: 'start' }}>
      ...
      <Card title="Weight Distribution">
        <div style={{ display: 'flex', gap: '6px', alignItems: 'flex-start' }}>
          <div style={{ flex: 1 }}>
            <WeightHeatmap fl={loadCells.fl} fr={loadCells.fr} rl={loadCells.rl} rr={loadCells.rr} cogX={cogX} cogY={cogY} />
          </div>
          <HeatLegend maxVal={Math.max(loadCells.fl, loadCells.fr, loadCells.rl, loadCells.rr, 0.001)} />
        </div>
        <div style={{ fontSize: '10px', fontFamily: theme.monoFont, color: theme.muted, textAlign: 'center', marginTop: '8px' }}>
          CoM: X {fmt(cogX, 1)}% &nbsp; Y {fmt(cogY, 1)}%
        </div>
      </Card>
